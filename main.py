...
# Intenta importar asyncio, si falla, importa uasyncio (común en MicroPython)
try:
    import asyncio
except ImportError:
...
# ----------------------------------------
# Configuración de Pines y PWM (MODIFICADO)
# ----------------------------------------
# --- Chasis (Salida Digital) ---
...
# --- Motores de Funciones (Salida PWM) ---
# Frecuencia para los motores DC
PWM_FREQ = 1000 
# Variable global para la velocidad (0-1023)
pwm_speed = 512 # Inicia a media velocidad
...
def stop_all_chassis():
    """Detiene solo los motores del chasis."""
    for pin in CHASSIS_PINS:
        pin.off()

def stop_all_pwm():
    """Detiene solo los motores PWM."""
    for pwm_pin in PWM_MOTOR_PINS:
        pwm_pin.duty(0)

def stop_all():
    """Detiene todos los motores y el LED."""
    stop_all_chassis()
    stop_all_pwm()
    LED_PIN.off()
...
def connect_to_wifi(ssid, password, timeout_s=15):
    """Conecta el ESP32 a una red WiFi existente."""
...
# ----------------------------------------
# Página de control HTML (VERSIÓN CORREGIDA PARA ERRORES JS Y CSS)
# ----------------------------------------
HTML_PAGE = """
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        body { font-family: Arial, sans-serif; background-color: #2c3e50; color: #ecf0f1; text-align: center; margin: 0; padding: 0; }
        .section { background-color: #34495e; border-radius: 12px; padding: 15px; margin: 10px auto; max-width: 320px; box-shadow: 0 4px 8px rgba(0,0,0,0.3); }
        h1 { color: #3498db; }
        h2 { color: #95a5a6; margin-top: 0; }
        .btn { border: none; color: white; padding: 15px 20px; font-size: 16px; margin: 6px; border-radius: 8px; cursor: pointer; transition: background-color 0.2s; width: 120px; }
        .btn-forklift { background-color: #e67e22; }
        .btn-camera { background-color: #1abc9c; }
        .btn-led { background-color: #f1c40f; }
        .btn-stop { background-color: #e74c3c; width: 260px; font-weight: bold; }
        .btn:active { transform: scale(0.95); }
        #joystick-container { position: relative; width: 200px; height: 200px; margin: 20px auto; background-color: #2c3e50; border-radius: 50%; }
        .slider-container { margin-top: 20px; }
        .slider { 
            -webkit-appearance: none; /* <-- Propiedad para Chrome/Safari */
            appearance: none; /* <-- CORRECCIÓN: Propiedad estándar para compatibilidad */
            width: 80%; 
            height: 15px; 
            background: #566573; 
            outline: none; 
        ...
        .slider::-moz-range-thumb { width: 25px; height: 25px; background: #3498db; cursor: pointer; border-radius: 50%; }
    </style>
</head>
<body>
    ...
    <div class="section">
        <h2>Movimiento Chasis</h2>
        <div id="joystick-container"></div>
    </div>
    
    <div class="section">
        <h2>Velocidad PWM (Funciones)</h2>
        <div class="slider-container">
            <input type="range" min="100" max="1023" value="512" class="slider" id="speedSlider">
            <p>Valor: <span id="speedValue">512</span></p>
        </div>
    </div>

    <div class="section">
        <h2>Montacargas (3s)</h2>
        <button class="btn btn-forklift" onclick="sendCommand('forklift_up')">Subir</button>
        <button class="btn btn-forklift" onclick="sendCommand('forklift_down')">Bajar</button>
    </div>

    <div class="section">
        <h2>Cámara (1s)</h2>
        <button class="btn btn-camera" onclick="sendCommand('cam_a')">Rotar A</button>
        <button class="btn btn-camera" onclick="sendCommand('cam_b')">Rotar B</button>
    </div>

    <div class="section">
        <h2>Utilidades</h2>
        <button class="btn btn-led" onclick="sendCommand('led_on')">LED ON</button>
        <button class="btn btn-led" onclick="sendCommand('led_off')">LED OFF</button>
    </div>

    <div class="section">
        <h2>Control Global</h2>
        <button class="btn btn-stop" onclick="sendCommand('stop')">DETENER TODO</button>
    </div>
    ...
    <script>
        ...
        joystick.on('move', function (evt, data) {
            if (data.direction) {
                ...
                if (direction !== lastDirection) {
                    if (direction === 'up') sendCommand('forward');
                    else if (direction === 'down') sendCommand('backward');
                    else if (direction === 'left') sendCommand('left');
                    else if (direction === 'right') sendCommand('right');
        ...
        joystick.on('end', function (evt) {
            if (lastDirection !== null) {
                sendCommand('stop_chassis');
                lastDirection = null;
                console.log('Joystick liberado, deteniendo chasis.');
            }
        });
        ...
        speedSlider.oninput = function() {
            ...
            sendCommand('set_speed', this.value);
        }
    </script>
</body>
</html>
...
# ----------------------------------------
# Servidor Web Asíncrono (MODIFICADO)
# ----------------------------------------
async def handle_client(reader, writer):
    ...
    try:
        ...
        # Leer el resto de los encabezados
        while await reader.readline() != b"\r\n": pass
        
        # Mapeo de comandos síncronos y asíncronos
        sync_commands = {
            '/forward': move_forward,
            '/backward': move_backward,
            '/left': turn_left,
            '/right': turn_right,
            '/stop_chassis': stop_all_chassis, # Comando específico del joystick
            '/stop': stop_all,
            '/led_on': led_on,
            '/led_off': led_off
        }
        ...
        print(f"⚠️ Error manejando cliente: {e}")
    finally:
        writer.close()
        await writer.wait_closed()
...
# ----------------------------------------
# Función Principal del Programa (MODIFICADA)
# ----------------------------------------
async def main():
    stop_all()
    ...
    if not ip_address:
        print("Error crítico: No se pudo conectar a la red WiFi. Reinicia la placa para reintentar.")
        return

    print("\n--- INSTRUCCIONES ---")
    print("Joystick virtual para mover el chasis.")
    print("Usa el slider para ajustar la velocidad de las funciones (montacargas/cámara).")
    print(f"Para controlar el robot, abre un navegador y ve a:")
    ...
    try:
        ...
        print("✅ Servidor iniciado. El robot está listo para recibir comandos.")
        while True:
        ...
        stop_all()
        print("Programa finalizado, todos los motores detenidos.")
